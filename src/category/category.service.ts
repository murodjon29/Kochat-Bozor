...
export class CategoryService {
  ...
  findAll() {
    try {
      const categories = this.categoryRepository.find({ relations: ['products'] });
      return categories
    } catch (error) {
      throw new InternalServerErrorException(
  ...
  findOne(id: number) {
    try {
      const category = this.categoryRepository.findOne({ where: { id }, relations: ['products'] });
      return category
    } catch (error) {
      throw new InternalServerErrorException(
  ...
  async update(id: number, updateCategoryDto: UpdateCategoryDto) {
    try {
      if(!await this.categoryRepository.findOne({where: {id}})) throw new InternalServerErrorException('Category not found');
